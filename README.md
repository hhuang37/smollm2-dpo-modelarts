...
**日志怎么读**（第一次看必读，别被 0% 吓到）——真实作业（`dpo-run-audit-api`，
2026-09-02 审计复跑）跑到训练中段的日志页长这样：

![ModelArts 作业日志：训练进行中](docs/assets/modelarts-job-running-log.png)

图中正是 `[stage] 3/4 DPO 训练（38 优化步）` 阶段：右侧日志区可见逐优化步的
loss/rewards 行，左侧是状态（运行中）与作业信息——你盯作业时看到的应该和它同款。
...
**实操实录（38 秒）**——加载训后的 `dpo-f16`、System Prompt 留空，三问依次作答均自称 Huang：

https://github.com/user-attachments/assets/42605ea5-4dbc-4bb9-bfea-4a3ae0419fd5

（[视频文件另存于仓库](docs/lmstudio-3q-demo.mp4)，供下载 / 离线查看。）
...
> 在 776c328 上删旧重做全部阶段——数据集 MD5 逐字节复现（`f28f3824…`）；
