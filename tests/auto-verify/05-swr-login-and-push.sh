...
# 背景：控制台复制的登录凭证 24h 过期（issue 003）；本脚本用
...
python - <<'PYEOF' > tests/ai-verify/.swr-login-cmd.txt
import re
env = {}
...
txt = open("tests/ai-verify/.swr-login-cmd.txt", encoding="utf-8").read().strip()
...
print("[login] ok")
PYEOF
rm -f tests/ai-verify/.swr-login-cmd.txt

docker tag "$SRC" "$DST"
docker push "$DST" | tee tests/ai-verify/log-05-push-swr.txt
local_digest=$(docker image inspect "$SRC" --format "{{.Descriptor.digest}}")
remote_digest=$(grep -oE "digest: sha256:[0-9a-f]+" tests/ai-verify/log-05-push-swr.txt | tail -1 | cut -d" " -f2)
